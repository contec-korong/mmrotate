...
class GeoAIDataset(CustomDataset):
    ...
    def __init__(self,
        ...
        super(GeoAIDataset, self).__init__(ann_file, pipeline, **kwargs)
    ...
    def load_annotations(self, ann_folder):
        ...
        ann_files = [os.path.join(ann_folder, ann_file) for ann_file in os.listdir(ann_folder)]
        data_infos = []
        ...
        #             data_infos.append(data_info)
            ...
            for idx, ann_file in enumerate(ann_files):
                ...
                data_info = {}
                data_info['filename'] = j['annotation']['filename']
                data_info['ann'] = {}
                gt_bboxes = []
                gt_labels = []
                gt_polygons = []
                ...
                if isinstance(j['annotation']['object'], dict):
                    j['annotation']['object'] = [j['annotation']['object']]

                for obj in j['annotation']['object']:
                    rbbox = obj['robndbox']
                    x, y, w, h, a = np.array([rbbox['cx'], rbbox['cy'],
                                              rbbox['w'], rbbox['h'],
                                              rbbox['angle']], dtype=np.float32)
                    poly = np.array([rbbox['x1'], rbbox['y1'],
                                     rbbox['x2'], rbbox['y2'],
                                     rbbox['x3'], rbbox['y3'],
                                     rbbox['x4'], rbbox['y4']], dtype=np.float32)
